...
def compute_mean_std(intervals, bdf,  stim, fs, fname_bdf, fs_bdf, fname_wav):
    ...
    if stim:
        ...
        # Случай: только стимул — получаем индексы и сохраняем в CSV
...
def plot_deviations(ax, intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start):
    m = min(len(intervals_wav_s_cum_with_start), len(intervals_bdf_s_cum_with_start))
    diff_intervals = 1000 * (intervals_wav_s_cum_with_start[:m] - intervals_bdf_s_cum_with_start[:m])
    data = np.array(diff_intervals)
    ...
    # Размер локального окна вокруг моды
    window_ratio = 0.08
...
def prepare_intervals(intervals_bdf_s, intervals_wav_s):
    ...
    intervals_bdf_s_cum = np.cumsum(intervals_bdf_s)
    intervals_bdf_s_cum_with_start = np.concatenate([[0], intervals_bdf_s_cum])
    ...
    return intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start, stim_wav, stim_bdf

def plot_triggers(ax2, ax3, ax4, intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start, stim_wav,
                  stim_bdf, filename_bdf, filename_wav):
    ax2.vlines(
    ...
    for xmin_i in stim_wav_trim:
        ax2.hlines(
            y=0.4,  # уровень линии соответствует значению stim_bdf
    ...
    ax3.vlines(intervals_bdf_s_cum_with_start[:trim_interval], ymin=0, ymax=0.7,
    ...
    for xmin_i in stim_bdf_trim:
        ax3.hlines(
            ...
            xmin=intervals_bdf_s_cum_with_start[xmin_i],
            xmax=intervals_bdf_s_cum_with_start[xmin_i + 1],
            colors='blue',
            linewidth=2,
            alpha=0.6
        )
    ...
    # BDF данные — синие
    ax4.vlines(
        intervals_bdf_s_cum_with_start[:trim_interval],
        ymin=0,
        ymax=0.7,
        colors='blue',
        linewidth=2,
...
def plot_intervals(dummy, intervals_bdf_s: object, intervals_wav_s: object, filename_bdf, filename_wav) -> None:
    """
    Plots intervals_bdf_ms, intervals_wav_ms on one scale
    """

    (intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start,
     stim_wav, stim_bdf) = prepare_intervals(intervals_bdf_s, intervals_wav_s)
    ...
    title = plot_deviations(ax1, intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start)
    ax1.set_title(title, fontsize=16)

    plot_triggers(ax2, ax3, ax4, intervals_wav_s_cum_with_start, intervals_bdf_s_cum_with_start,
    ...
    # Добавляем текст на новую ось в первом столбце
...
def compute_interval_stat(dummy, events_wav, events_bdf, stat, fs_bdf, fs_wav, fname_bdf, fname_wav):
    ...
    plot_intervals(dummy, intervals_bdf_s, intervals_wav_s, fname_bdf, fname_wav)
